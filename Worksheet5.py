...
x = x + np.random.normal(0, 0.3, len(t))
b, a = signal.butter(4, 3/(fs/2), btype='low')  
...
import numpy as np
import matplotlib.pyplot as plt
np.random.seed(0)  # for same random values each run
months = np.array(['Jan','Feb','Mar','Apr','May','Jun',
                   'Jul','Aug','Sep','Oct','Nov','Dec'])
products = ['Product A','Product B','Product C','Product D']
# Create 12×4 random sales data (in thousands)
sales = np.random.randint(20, 100, size=(12, 4))
# Step 2: Calculate statistics
total_by_product = np.sum(sales, axis=0)
avg_by_product   = np.mean(sales, axis=0)
max_by_product   = np.max(sales, axis=0)
min_by_product   = np.min(sales, axis=0)
# Total sales for each month (sum across products)
total_by_month = np.sum(sales, axis=1)
best_month_index  = np.argmax(total_by_month)
worst_month_index = np.argmin(total_by_month)
# Step 3: Print results
print("Sales Data (in thousands):\n", sales)
print("\nTotal by Product:", total_by_product)
print("Average by Product:", np.round(avg_by_product, 2))
print("Maximum by Product:", max_by_product)
print("Minimum by Product:", min_by_product)
print("\nBest Month :", months[best_month_index], "with total =", total_by_month[best_month_index])
print("Worst Month:", months[worst_month_index], "with total =", total_by_month[worst_month_index])
# Step 4: Plot graphs
# Bar chart for products
plt.figure(figsize=(10,4))
x = np.arange(len(products))
plt.bar(x, total_by_product, color='skyblue')
plt.xticks(x, products)
plt.title("Total Sales per Product (Yearly)")
plt.xlabel("Product")
plt.ylabel("Total Sales (thousands)")
plt.show()
# Line chart for monthly total sales
plt.figure(figsize=(10,4))
plt.plot(months, total_by_month, marker='o', color='orange')
